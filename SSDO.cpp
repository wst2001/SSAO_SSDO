...
int main()
{
	...
	if (SS_mode == 0) {
		Shader shaderLightingPass("../GLSL/ssao_pass.vs", "../GLSL/ssao_lighting.fs");
		Shader shaderSSAO("../GLSL/ssao_pass.vs", "../GLSL/ssao.fs");
		Shader shaderSSAOBlur("../GLSL/ssao_pass.vs", "../GLSL/ssao_blur.fs");
	}
	else if (SS_mode == 1) {
	...
	vector<unsigned int*> FBOs;
	vector<unsigned int*> Texs;
	if (SS_mode == 0) {
		// 创建SSAO缓冲
		// -----------------------------------------------------
		unsigned int
		...
		FBOs = vector<unsigned int*>{
			&ssaoFBO,
			&ssaoBlurFBO,
			&skyboxFBO,
		};
		Texs = vector<unsigned int*>{
			&ssaoTex,
			&ssaoBlurTex,
			&skyboxTex,
		};
